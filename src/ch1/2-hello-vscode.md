...
Along the top you should see `[] File`,`[] Edit`,`[] Selection`, etc. Among these only `[] File` is really important if you click on it a menu will pop down in this menu you can find `[] Open File`,`[] Open Folder` and `[] Preferences`. You can use `[] Open File` to open files to edit using vscode, `[] Open Folder` is similar for for opening folders. Lastly `[] Preferences` is where you can edit your setting and configure vscode to your liking. One of these setting you might want is `[] Auto Save` to automatically save your file.
